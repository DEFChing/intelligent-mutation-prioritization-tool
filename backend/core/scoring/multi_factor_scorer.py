"""
Multi-Factor Scorer for IMPT System

Scores mutants based on multiple weighted factors to prioritize execution.
"""
...
from core.models import CodeMetrics, HistoricalData, Mutant, MutationOperator
...
class MultiFactorScorer:
    ...
    def _precompute_metrics(self, mutants: List[Mutant]):
        ...
        for mutant in mutants:
            # Use code_analyzer.analyze() which accepts Mutant object
            # This works with both mock and real file analysis
            mutant.code_metrics = self.code_analyzer.analyze(mutant)
    ...
    def _compute_normalized_factors(
        ...
        for i, mutant in enumerate(mutants):
            result[mutant.id] = {
                factor: normalized[factor][i] for factor in raw_factors.keys()
            }

        return result

    def _calculate_weighted_score(
        self, mutant: Mutant, normalized: Dict[str, Dict[str, float]]
    ) -> float:
        """
        ...
        weighted_sum = sum(
            self.WEIGHTS[factor] * factors[factor] for factor in factors.keys()
        )
    ...
    def _weighted_sum(self, factors: Dict[str, float]) -> float:
        ...
        return sum(self.WEIGHTS[key] * value for key, value in factors.items()) * 10.0
    ...
    def _complexity_factor(mutant: Mutant) -> float:
        ...
        # Note: number_of_parameters might not always be available, default to 0
        num_params = getattr(metrics, 'number_of_parameters', 0)
    ...
    def _bug_history_factor(self, mutant: Mutant) -> float:
        ...
        """
        # For demo: Use mock data based on file characteristics
        # In production: Would query actual bug tracking system
        
        file_path = mutant.location.file_path.lower()
        
        # Simulate higher bug counts for certain file types
        if 'auth' in file_path or 'security' in file_path:
            bug_count = 8  # Auth files tend to have more bugs
        elif 'service' in file_path:
            bug_count = 5  # Service files moderate bugs
        elif 'util' in file_path or 'helper' in file_path:
            bug_count = 3  # Utilities fewer bugs
        elif 'model' in file_path or 'dto' in file_path:
            bug_count = 1  # Data classes fewest bugs
        else:
            bug_count = 4  # Default
        ...
        return {
            ...
            "breakdown": {
                "historical_effectiveness": {
                    ...
                    "weighted_contribution": factors["historical"]
                ...
                "code_complexity": {
                    ...
                    "weighted_contribution": factors["complexity"]
                ...
                "security_criticality": {
                    ...
                    "weighted_contribution": factors["security"]
                ...
                "change_recency": {
                    ...
                    "weighted_contribution": factors["recency"]
                ...
                "bug_history": {
                    ...
                    "weighted_contribution": factors["bug_history"]
                    * self.WEIGHTS["bug_history"]
                    * 10,
